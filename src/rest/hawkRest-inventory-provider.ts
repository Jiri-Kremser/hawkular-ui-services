...
_module.provider('HawkularInventory', function() {
  ...
  this.$get = ['$resource', '$location', '$rootScope', '$log',
    function($resource, $location, $rootScope, $log): IHawkularInventory {
    ...
    factory.Events = (tenantId) => {
      return {
        listen: (handler: IWebSocketHandler): IWebSocketDisposable => {
          ...
          // the consumer can call .dispose() on it to clear the resource
          const disposable: IWebSocketDisposable = {
            dispose: ws.close
          };
          return disposable;
        }
    ...
    factory.RxEvents = (tenantId: string): Rx.Observable<any> => {
      // create a web socket subject
      const subject = Rx.Observable.webSocket(wsUrl + `?tenantId=${tenantId}`);
      return subject;
    };
